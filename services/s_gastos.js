...
async function getResumoAno(){   
    ...
    let sql =  `SELECT SUM(g.valor) as valor,(SELECT MONTH(g.dt_vencimento)) as mesVencimento,(SELECT YEAR(g.dt_vencimento)) as ano,m.descricao as motivo,inOrcamentario
    FROM Gastos as g
    INNER JOIN MotivoGastos as m ON m.id = g.motivo
    WHERE g.usuario = ?
